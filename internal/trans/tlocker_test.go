...
func initTLTest(t *testing.T) (*Locker, tlTestCtx) {
	t.Helper()
	b := memory.New()
	return newTestLocker(t, b)
}

func newTestLocker(t *testing.T, b backend.Backend) (*Locker, tlTestCtx) {
	t.Helper()
	l := storage.NewLocal(cache.New(1024))
	g := storage.NewGlobal(b, l)
...
func TestLockReadRemote(t *testing.T) {
	synctest.Test(t, func(t *testing.T) {
		...
		locker2, tctx2 := newTestLocker(t, tctx1.Backend)
