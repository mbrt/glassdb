...
func TestCleanAbort(t *testing.T) {
	...
	ctx := context.Background()
	// Shared backend across subtests; fresh algos per synctest bubble so
	// Background/WaitGroup are not used from multiple bubbles.
	sharedBackend := memory.New()

	for numWrites := 1; numWrites <= 3; numWrites++ {
		t.Run(fmt.Sprintf("%d writes", numWrites), func(t *testing.T) {
			synctest.Test(t, func(t *testing.T) {
				tm, tctx := newAlgoFromBackend(t, sharedBackend)
				tm2, _ := newAlgoFromBackend(t, sharedBackend)

				keys := make([]string, numWrites)
				for i := 0; i < numWrites; i++ {
					keys[i] = paths.FromKey("testp", fmt.Appendf(nil, "k%d", i))
				}
